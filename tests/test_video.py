...
def test_trim(sample_video):
    ...
    trimmed = sample_video.trim(
        start_time=1 / sample_video.fps,
        end_time=sample_video.duration - 1 / sample_video.fps,
    )
...
def test_create_transition(sample_frames):
    ...
    transition = Video.create_transition(
        video1, video2, transition_frames=5, transition_type="crossfade"
    )
...
def test_apply_filter_to_frame_range(sample_video):
    ...
    end_idx = 5
    original_frames = [
        frame.array.copy() for frame in sample_video.frames[start_idx:end_idx]
    ]

    # Apply filter to range
    filtered = sample_video.apply_filter_to_frame_range(
        "sharpen", start_frame=start_idx, end_frame=end_idx
    )

    # Verify frames in range were modified
    for i in range(start_idx, end_idx):
        assert not np.array_equal(
            filtered.frames[i].array, original_frames[i - start_idx]
        )
...
def test_create_loop(sample_video):
    ...
    # Test smooth looping
    smooth_loop = sample_video.create_loop(
        repetitions=2, smooth_transition=True, transition_frames=5
    )
