...
from llama_canvas.core.agent_manager import Agent, AgentManager, ClaudeAgent
...
class TestClaudeAgent:
    ...
    def test_generate_image(self, mock_anthropic, mock_canvas, sample_image):
        ...
        mock_message.content = [
            MagicMock(
                type="image",
                source=MagicMock(media_type="image/png", data=b"test_image_data"),
            )
        ]
        ...
        with patch("PIL.Image.open", return_value=sample_image._image), patch.dict(
            ...
            assert (
                "Create a sunset over mountains" in call_args["messages"][0]["content"]
            )
...
class TestAgentManager:
    ...
    def test_apply_style(self, agent_manager, sample_image):
        ...
        result = agent_manager.apply_style(
            sample_image, "test style", agent="test_agent"
        )

        # Verify agent called correctly
        mock_agent.style_image.assert_called_once_with(
            sample_image, "test style", strength=0.8
        )
        assert result is sample_image
    ...
    def test_enhance_resolution(self, agent_manager, sample_image):
        ...
        result = agent_manager.enhance_resolution(
            sample_image, scale=2, agent="test_agent"
        )
