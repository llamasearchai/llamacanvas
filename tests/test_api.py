...
def test_style_image_endpoint(api_client, sample_base64_image):
    ...
    with patch("llama_canvas.api.routes.images.Canvas") as mock_canvas_cls, patch(
        "llama_canvas.api.routes.images.Image"
    ) as mock_image_cls:
...
def test_enhance_image_endpoint(api_client, sample_base64_image):
    ...
    with patch("llama_canvas.api.routes.images.Canvas") as mock_canvas_cls, patch(
        ...
        # Make API request
        response = api_client.post(
            "/images/enhance", json={"image": sample_base64_image, "scale": 2}
        )
...
def test_apply_filter_endpoint(api_client, sample_base64_image):
    ...
    with patch("llama_canvas.api.routes.images.Image") as mock_image_cls:
        ...
        # Make API request
        response = api_client.post(
            "/images/filter", json={"image": sample_base64_image, "filter": "blur"}
        )
...
def test_resize_image_endpoint(api_client, sample_base64_image):
    ...
    with patch("llama_canvas.api.routes.images.Image") as mock_image_cls:
        ...
        response = api_client.post(
            "/images/resize", json={"image": sample_base64_image, "scale": 0.5}
        )
...
def test_rotate_image_endpoint(api_client, sample_base64_image):
    ...
    with patch("llama_canvas.api.routes.images.Image") as mock_image_cls:
        ...
        response = api_client.post(
            "/images/rotate",
            json={"image": sample_base64_image, "angle": 90, "expand": True},
        )
...
def test_create_animation_endpoint(api_client, sample_base64_image):
    ...
    with patch("llama_canvas.api.routes.videos.Video") as mock_video_cls, patch(
        "llama_canvas.api.routes.videos.Image"
    ) as mock_image_cls:
        ...
        with patch("os.path") as mock_path, patch(
            ...
            with patch("builtins.open", MagicMock()) as mock_open:
                ...
                response = api_client.post(
                    "/videos/create",
                    json={"frames": frames_base64, "fps": 24, "loop": 0},
                )
