...
def test_generate_command(cli_runner, temp_output_path):
    ...
    with patch("llama_canvas.cli.Canvas") as mock_canvas_cls:
        ...
        mock_canvas_cls.assert_called_once_with(
            width=256, height=256, background_color=(255, 255, 255)
        )
...
def test_style_command(cli_runner, temp_image_path, temp_output_path):
    ...
    with patch("llama_canvas.cli.Image") as mock_image_cls, patch(
        "llama_canvas.cli.Canvas"
    ) as mock_canvas_cls:
...
def test_enhance_command(cli_runner, temp_image_path, temp_output_path):
    ...
    with patch("llama_canvas.cli.Image") as mock_image_cls, patch(
        ...
        result = cli_runner.invoke(
            cli,
            ["enhance", temp_image_path, "--output", temp_output_path, "--scale", "2"],
        )
...
def test_animation_command(cli_runner, temp_dir, temp_output_path):
    ...
    with patch("llama_canvas.cli.Image") as mock_image_cls, patch(
        "llama_canvas.cli.Canvas"
    ) as mock_canvas_cls, patch("llama_canvas.cli.Video") as mock_video_cls, patch(
        "llama_canvas.cli.glob"
    ) as mock_glob:
...
def test_filter_command(cli_runner, temp_image_path, temp_output_path):
    ...
    with patch("llama_canvas.cli.Image") as mock_image_cls:
        ...
        # Run the command
        result = cli_runner.invoke(
            cli, ["filter", temp_image_path, "blur", "--output", temp_output_path]
        )
...
def test_ui_command(cli_runner):
    """Test the UI command."""
    with patch("llama_canvas.cli.subprocess") as mock_subprocess, patch(
        "llama_canvas.cli.webbrowser"
    ) as mock_webbrowser:
...
def test_rotate_command(cli_runner, temp_image_path, temp_output_path):
    ...
    with patch("llama_canvas.cli.Image") as mock_image_cls:
        ...
        result = cli_runner.invoke(
            cli,
            ["rotate", temp_image_path, "90", "--output", temp_output_path, "--expand"],
        )
...
def test_resize_command(cli_runner, temp_image_path, temp_output_path):
    ...
    with patch("llama_canvas.cli.Image") as mock_image_cls:
        ...
        result = cli_runner.invoke(
            cli,
            ["resize", temp_image_path, "--output", temp_output_path, "--scale", "0.5"],
        )
