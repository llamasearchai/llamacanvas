    ...
    def test_save_audio(self, sample_audio_data):
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
            ...
            with patch("llama_canvas.audio.sf") as mock_sf:
                ...
                save_audio(
                    audio_data, temp_path, sample_rate=sample_rate, subtype="PCM_24"
                )
...
class TestAudioManipulation:
    ...
    def test_convert_audio_format(self, sample_audio_data, tmp_path):
        ...
        with patch("llama_canvas.audio.sf") as mock_sf:
            ...
            with patch("llama_canvas.audio.subprocess") as mock_subprocess:
                ...
                # Convert format
                result_path = convert_audio_format(
                    input_path, output_format="mp3", output_path=output_path
                )
    ...
    def test_add_fade(self, sample_audio_data):
        ...
        with pytest.raises(ValueError):
            add_fade(
                audio_data, sample_rate, fade_in_sec=1.5, fade_out_sec=1.5
            )  # Total > duration
    ...
    def test_apply_audio_filter(self, sample_audio_data):
        ...
        with patch("llama_canvas.audio.scipy.signal") as mock_signal:
            ...
            filtered = apply_audio_filter(
                audio_data, sample_rate, filter_type="lowpass", cutoff_freq=1000
            )
            ...
            filtered = apply_audio_filter(
                audio_data, sample_rate, filter_type="highpass", cutoff_freq=500
            )
            ...
            mock_signal.butter.reset_mock()
            filtered = apply_audio_filter(
                audio_data, sample_rate, filter_type="bandpass", cutoff_freq=[500, 2000]
            )
            ...
            with pytest.raises(ValueError):
                apply_audio_filter(
                    audio_data, sample_rate, filter_type="bandpass", cutoff_freq=1000
                )
...
class TestAudioAnalysis:
    ...
    def test_generate_spectrogram(self, sample_audio_data):
        ...
        with patch("llama_canvas.audio.plt") as mock_plt, patch(
            "llama_canvas.audio.librosa.display"
        ) as mock_display:
            ...
            with tempfile.NamedTemporaryFile(suffix=".png") as temp_file:
                ...
                spec_path = generate_spectrogram(
                    audio_data, sample_rate, output_path=temp_file.name, as_image=True
                )
            ...
            spec_custom = generate_spectrogram(
                audio_data, sample_rate, n_fft=1024, hop_length=256, fmin=50, fmax=8000
            )
    ...
    def test_analyze_audio(self, sample_audio_data):
        ...
        analysis_custom = analyze_audio(
            audio_data, sample_rate, frame_size=2048, hop_length=1024
        )
    ...
    def test_detect_silence(self, sample_audio_data):
        ...
        silence_high_thresh = detect_silence(
            audio_with_silence, sample_rate, threshold=0.1
        )
        ...
        silence_min_dur = detect_silence(
            audio_with_silence, sample_rate, min_duration=1.0
        )
...
class TestAudioTranscription:
    ...
    def test_transcribe_audio(self, sample_audio_path):
        ...
        with patch("llama_canvas.audio.sr") as mock_sr:
            ...
            assert (
                mock_sr.Recognizer.return_value.recognize_google.call_args[1][
                    "language"
                ]
                == "fr-FR"
            )
