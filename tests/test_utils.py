...
from llama_canvas.utils.validators import validate_aspect_ratio, validate_image_size
...
class TestLogging:
    ...
    def test_setup_logging_with_file(self):
        ...
        with patch("logging.FileHandler") as mock_file_handler, patch(
        ...
        ) as mock_stream_handler, patch("logging.getLogger") as mock_get_logger:
            ...
            suppressed_loggers = [
                args[0][0]
                for args in suppressed_calls
                if args[0][0] in ("PIL", "urllib3", "matplotlib")
            ]
...
class TestConfig:
    ...
    def test_settings_init(self):
        ...
        with patch(
            "llama_canvas.utils.config.Settings._load_environment"
        ) as mock_load_env, patch(
            "llama_canvas.utils.config.Settings._load_config_file"
        ) as mock_load_config:
    ...
    def test_settings_save_error(self):
        ...
        with patch("builtins.open", side_effect=IOError("Test error")), patch(
            "builtins.print"
        ) as mock_print, patch("pathlib.Path.mkdir"):
