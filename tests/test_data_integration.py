...
class TestDataLoading:
    ...
    def sample_json_path(self, tmp_path):
        ...
        data = {
            ...
            "metadata": {"source": "test", "created": "2023-04-01"},
        }
    ...
    def test_load_database(self):
        ...
        with patch("llama_canvas.data.create_engine") as mock_create_engine:
            ...
            with patch("llama_canvas.data.pd.read_sql") as mock_read_sql:
                ...
                data_table = load_database(
                    table="users", connection_string="sqlite:///test.db"
                )
...
class TestDataProcessor:
    ...
    def test_filter_data(self, sample_dataframe):
        ...
        filtered_both = processor.filter_data(
            lambda df: df["category"] == "A", columns=["category", "numeric"]
        )
    ...
    def test_transform_data(self, sample_dataframe):
        ...
        transformed = processor.transform_data(
            lambda df: df.assign(doubled=df["numeric"] * 2)
        )
        ...
        transforms = {"numeric": lambda x: x + 10, "category": lambda x: x.str.lower()}
        ...
        assert (
            transformed_cols["category"] == sample_dataframe["category"].str.lower()
        ).all()
    ...
    def test_aggregate_data(self, sample_dataframe):
        ...
        aggregated = processor.aggregate_data(
            group_by="category", aggregations={"numeric": ["mean", "sum"]}
        )
...
class TestDataVisualizer:
    ...
    def test_create_chart(self, sample_dataframe):
        ...
        with patch("llama_canvas.data.plt") as mock_plt, patch(
            "llama_canvas.data.sns"
        ) as mock_sns:

            # Test bar chart
            visualizer.create_chart(x="category", y="y", chart_type="bar")
            ...
            mock_sns.reset_mock()
            visualizer.create_chart(x="x", y="y", chart_type="line")
    ...
    def test_overlay_data_on_image(self):
        ...
        data_points = pd.DataFrame(
            {"x": [25, 50, 75], "y": [25, 50, 75], "value": [1, 2, 3]}
        )

        with patch("llama_canvas.data.plt") as mock_plt, patch(
            "llama_canvas.data.PILImage"
        ) as mock_pil:
            ...
            result = overlay_data_on_image(
                image=img, data=data_points, x_col="x", y_col="y", value_col="value"
            )
