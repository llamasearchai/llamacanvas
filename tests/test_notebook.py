...
class TestNotebookDisplay:
    ...
    def test_create_canvas(self, mock_display):
        ...
        with patch("llama_canvas.notebook.Canvas") as MockCanvas, patch(
            "llama_canvas.notebook.HTML"
        ) as mock_html:
    ...
    def test_create_interactive_canvas(self, mock_display):
        ...
        with patch("llama_canvas.notebook.widgets") as mock_widgets, patch(
            "llama_canvas.notebook.Canvas"
        ) as MockCanvas:
    ...
    def test_create_grid(self, sample_image, mock_display):
        ...
        with patch("llama_canvas.notebook.Image") as mock_image, patch(
            "llama_canvas.notebook.HTML"
        ) as mock_html:
...
class TestNotebookExport:
    ...
    def sample_notebook(self):
        ...
        return {
            "cells": [
                {"cell_type": "markdown", "source": ["# LlamaCanvas Test Notebook"]},
                {
                    "cell_type": "code",
                    "source": [
                        "import numpy as np\n",
    ...
    def test_export_notebook(self, sample_notebook, tmp_path):
        ...
        with patch("llama_canvas.notebook.get_ipython") as mock_get_ipython:
            ...
            with patch("llama_canvas.notebook.nbformat") as mock_nbformat:
                ...
                py_path = export_notebook(
                    format="python", output_path=tmp_path / "test.py"
                )
                ...
                html_path = export_notebook(
                    format="html", output_path=tmp_path / "test.html"
                )
                ...
                with patch("llama_canvas.notebook.subprocess.run") as mock_run:
                    ...
                    pdf_path = export_notebook(
                        format="pdf", output_path=tmp_path / "test.pdf"
                    )
