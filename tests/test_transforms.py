...
class TestImageTransforms:
    ...
    def sample_image(self):
        ...
        for y in range(height):
            for x in range(width):
                ...
                img[y, x, 1] = int(255 * y / height)   # Green increases from top to bottom
    ...
    def test_resize_image(self, sample_image):
        ...
        resized_nearest = resize_image(sample_image, width=50, height=50, interpolation="nearest")
        resized_bilinear = resize_image(sample_image, width=50, height=50, interpolation="bilinear")
    ...
    def test_rotate_image(self, sample_image):
        ...
        assert rotated.shape == (100, 100, 3)  # Shape should be the same for 90 degree rotation
        ...
        rotated_fill = rotate_image(sample_image, angle=45, expand=True, fill_color=(255, 0, 0))
    ...
    def test_convert_image_format(self, sample_image, tmp_path):
        ...
        gif_path = convert_image_format(png_path, output_format="GIF", output_path=custom_path)
        assert gif_path == custom_path
        assert os.path.exists(gif_path)
        
        # Test with quality parameter (JPEG)
        jpg_low_quality = convert_image_format(png_path, output_format="JPEG", quality=10)
        jpg_high_quality = convert_image_format(png_path, output_format="JPEG", quality=95)
...
class TestVideoTransforms:
    ...
    def test_resize_video(self, sample_video_path):
        ...
        with patch('llama_canvas.utils.transforms.subprocess.run') as mock_run:
            ...
            output_path = resize_video(sample_video_path, width=320, height=240, output_path=custom_path)
    ...
    def test_crop_video(self, sample_video_path):
        ...
        with patch('llama_canvas.utils.transforms.subprocess.run') as mock_run:
            ...
            # Test basic crop
            output_path = crop_video(sample_video_path, x=100, y=100, width=400, height=300)
            ...
            # Test with custom output path
            custom_path = os.path.join(os.path.dirname(sample_video_path), "custom.mp4")
            output_path = crop_video(sample_video_path, x=50, y=50, width=200, height=200, output_path=custom_path)
            assert output_path == custom_path
    ...
    def test_trim_video(self, sample_video_path):
        ...
        with patch('llama_canvas.utils.transforms.subprocess.run') as mock_run:
            ...
            # Test basic trim
            output_path = trim_video(sample_video_path, start_time="00:00:10", end_time="00:00:20")
            ...
            # Test with custom output path
            custom_path = os.path.join(os.path.dirname(sample_video_path), "custom.mp4")
            output_path = trim_video(sample_video_path, start_time=5, end_time=10, output_path=custom_path)
            assert output_path == custom_path
    ...
    def test_extract_frame(self, sample_video_path):
        ...
        with patch('llama_canvas.utils.transforms.subprocess.run') as mock_run:
            ...
            output_path = extract_frame(sample_video_path, time=5, output_path=custom_path)
            assert output_path == custom_path
    ...
    def test_add_audio_to_video(self, sample_video_path, tmp_path):
        ...
        with patch('llama_canvas.utils.transforms.subprocess.run') as mock_run:
            ...
            output_path = add_audio_to_video(sample_video_path, audio_path, output_path=custom_path)
            assert output_path == custom_path
