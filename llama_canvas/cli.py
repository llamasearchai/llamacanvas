...
def generate_image(args):
    ...
    # Create canvas
    canvas = Canvas(width=args.width, height=args.height, use_agents=not args.no_agents)
...
def apply_style(args):
    ...
    styled = canvas.apply_style(
        image, args.style, strength=args.strength, use_claude=args.use_claude
    )
...
def enhance_image(args):
    ...
    # Enhance image
    enhanced = canvas.enhance_resolution(image, scale=args.scale)
...
def main(argv: Optional[List[str]] = None):
    ...
    subparsers = parser.add_subparsers(
        title="commands", dest="command", help="Command to run", required=True
    )
    ...
    generate_parser.add_argument(
        "--output", "-o", default="output.png", help="Output file path"
    )
    generate_parser.add_argument(
        "--width", "-w", type=int, default=512, help="Image width in pixels"
    )
    generate_parser.add_argument(
        "--height", "-h", type=int, default=512, help="Image height in pixels"
    )
    generate_parser.add_argument(
        "--model",
        "-m",
    ...
    generate_parser.add_argument(
        "--steps", "-s", type=int, default=50, help="Number of inference steps"
    )
    generate_parser.add_argument(
        "--no-agents", action="store_true", help="Disable agent system"
    )
    generate_parser.add_argument(
        "--open", action="store_true", help="Open the image after generation"
    )
    generate_parser.set_defaults(func=generate_image)

    # Apply style command
    style_parser = subparsers.add_parser("style", help="Apply a style to an image")
    style_parser.add_argument("input", help="Input image path")
    style_parser.add_argument("style", help="Style name or reference image path")
    style_parser.add_argument(
        "--output", "-o", default="styled_output.png", help="Output file path"
    )
    style_parser.add_argument(
        "--strength",
        "-s",
    ...
    style_parser.add_argument(
        "--use-claude", action="store_true", help="Use Claude for style transfer"
    )
    style_parser.add_argument(
        "--no-agents", action="store_true", help="Disable agent system"
    )
    style_parser.add_argument(
        "--open", action="store_true", help="Open the image after processing"
    )
    ...
    enhance_parser.add_argument(
        "--output", "-o", default="enhanced_output.png", help="Output file path"
    )
    enhance_parser.add_argument(
        "--scale", "-s", type=int, default=2, help="Upscaling factor"
    )
    enhance_parser.add_argument(
        "--no-agents", action="store_true", help="Disable agent system"
    )
    ...
    # UI command
    ui_parser = subparsers.add_parser("ui", help="Run the web UI")
    ...
    ui_parser.add_argument(
        "--browse", "-b", action="store_true", help="Open a browser window"
    )
