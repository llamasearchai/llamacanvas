...
class Video:
    ...
    def add_frame(self, frame: Image, position: Optional[int] = None) -> None:
        ...
        logger.debug(
            f"Added frame at position {position if position is not None else len(self.frames) - 1}"
        )
    ...
    def save_gif(
        self, path: Union[str, Path], loop: int = 0, optimize: bool = True
    ) -> None:
        """
    ...
    def save_mp4(
        self, path: Union[str, Path], codec: str = "libx264", bitrate: str = "1M"
    ) -> None:
        """
        ...
        try:
            ...
            logger.error(
                "moviepy is required to save as MP4. Install it with 'pip install moviepy'"
            )
            return
    ...
    def slice(self, start: int, end: Optional[int] = None) -> "Video":
        ...
        return Video(
            self.frames[start:end], fps=self.fps, width=self.width, height=self.height
        )
