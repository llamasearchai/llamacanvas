...
class ClaudeAgent(Agent):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if api_key:
            ...
            logger.warning(
                "Claude API key not found. Set CLAUDE_API_KEY env var or configure in settings."
            )
...
class StableDiffusionAgent(Agent):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if self.config.get("initialize", False):
                ...
                logger.debug(
                    "Stable Diffusion initialization deferred (set initialize=True to load at startup)"
                )

        except ImportError:
            logger.warning(
                "diffusers package not installed. Install with 'pip install diffusers torch'"
            )
    ...
    def _ensure_model_loaded(self, model_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            model_id = model_id or self.config.get(
    ...
    def _generate_image(
        ...
        **kwargs,
        ...
        try:
            ...
            if (
                hasattr(result, "nsfw_content_detected")
                and result.nsfw_content_detected[0]
            ):
    ...
    def _image_to_image(
        ...
        **kwargs,
        ...
        """
        logger.debug(
            f"Performing image-to-image with Stable Diffusion, prompt: {prompt}"
        )

        # For placeholder implementation
        if not self._ensure_model_loaded(kwargs.get("model_id")):
            logger.warning(
                "Stable Diffusion not available for image-to-image, returning filtered image"
            )
        ...
        try:
            ...
            model_id = kwargs.get(
                "model_id",
                self.config.get("model_id", "stabilityai/stable-diffusion-2-1"),
            )
    ...
    def _inpaint(
        ...
        **kwargs,
        ...
        if not self._ensure_model_loaded(kwargs.get("model_id")):
            logger.warning(
                "Stable Diffusion not available for inpainting, returning original image"
            )
            return image
...
class AgentManager:
    ...
    def _init_default_agents(self) -> None:
        ...
        if settings.get("claude_api_key"):
            self.register_agent(
                "claude", ClaudeAgent({"api_key": settings.get("claude_api_key")})
            )

        # Initialize Stable Diffusion with deferred loading
        self.register_agent(
            "stable_diffusion", StableDiffusionAgent({"initialize": False})
        )
    ...
    def generate_image(
        self, prompt: str, model: str = "stable-diffusion", **kwargs
    ) -> Optional[Image]:
        """
        ...
        if model.startswith("stable-diffusion"):
            ...
            return self.execute_task(
                "generate_image", capability="image_generation", prompt=prompt, **kwargs
            )

    def enhance_image(
        self, image: Image, method: str = "super_resolution", **kwargs
    ) -> Image:
        """
        ...
        if method == "super_resolution":
            ...
            return image.resize((image.width * 2, image.height * 2)).apply_filter(
                "sharpen"
            )
    ...
    def apply_style(self, image: Image, style: Union[str, Image], **kwargs) -> Image:
        ...
        """
        logger.debug(
            f"Applying style: {style if isinstance(style, str) else 'reference image'}"
        )
        ...
        if agent:
            prompt = (
                f"Image in the style of {style}"
                if isinstance(style, str)
                else "Transfer the style of the reference image"
            )
